...
int32_t Protocentral_ADS1220::Read_SingleShot_WaitForData(void)
{
    ...
    while(digitalRead(m_drdy_pin) == HIGH){} // delay until conversion finishes
    ...
    //Serial.print(SPI_Buff[0]);Serial.print(",");Serial.print(SPI_Buff[1]);Serial.print(",");Serial.print(SPI_Buff[2]);Serial.print(","); Serial.println();
    //Serial.print(SPI_Buff[0],BIN);Serial.print(",");Serial.print(SPI_Buff[1],BIN);Serial.print(",");Serial.print(SPI_Buff[2],BIN);Serial.print(","); Serial.println();

    //Serial.println(bit24,BIN);
    bit24 = SPI_Buff[0];
    //Serial.println(bit24,BIN);
    bit24 = (bit24 << 8) | SPI_Buff[1];
    //Serial.println(bit24,BIN);
    ...
    //Serial.print("bit24: "); Serial.print(bit24, BIN);

    if (bit24 >> 23 == 1) { // negative number, twos compliment!
        //Serial.println("Negative Number detected!");
        mResult32 = 0xFF000000 | bit24; // saturate the upper byte
    }
    else {mResult32 = bit24;}
    //bit24= ( bit24 << 8 );
    //Serial.print("bit24: "); Serial.print(bit24);
    //mResult32= ( bit24 << 8 ); // getting the sign bit into place
    //Serial.print(", new bit24: "); Serial.print(bit24);
    //mResult32 = mResult32 & 0x80000000; // a 32 bit number with just the first bit set, to keep just the sign
    //Serial.print(", signed 32: "); Serial.print(mResult32);
    //bit24 = bit24 & 0x7FFFFF; // a 23 bit number number, to strip just the sign
    //Serial.print(", unsigned 24: "); Serial.print(bit24);
    //mResult32 = mResult32 & bit24;
    //Serial.print(", final result: "); Serial.print(mResult32); Serial.println();
    //mResult32 = ( bit24 >> 8 );                      // Converting 24 bit two's complement to 32 bit two's complement
