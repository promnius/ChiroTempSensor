...
void checkButtons(){
  ...
  if (modeButton.getPressingEdge()==true){ // button pressed!
    ...
    intMode += 1; if (intMode > 3){intMode = 0;}
    ...
    if (modeButton.getDoubleTap() == true){
      ...
      intMode -= 1;  if (intMode < 0){intMode = 3;}
      intMode -= 1;  if (intMode < 0){intMode = 3;}
      calibrateSensorOffsets();
    }
  }
  if (modeButton.getStateTime() > 1500 && booCheckButtonHeld == true && modeButton.getState() == 1){
  ...
  if (modeButton.getStateTime() > 3000 && modeButton.getState() == 1){
